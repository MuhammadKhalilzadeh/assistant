...
export const habitModel = {
  async findAll(userId: string): Promise<Habit[]> {
    ...
    for (const row of result.rows) {
      const completionsResult = await pool.query<{ completed_date: Date }>(
        `SELECT completed_date FROM habit_completions WHERE habit_id = $1 AND user_id = $2 ORDER BY completed_date DESC`,
        [row.id, userId]
      );
  ...
  async create(userId: string, input: CreateHabitInput): Promise<Habit> {
    const result = await pool.query<{ id: string }>(
      `INSERT INTO habits (user_id, name, description, icon, category, frequency, target_days)
    ...
    const habit = await this.findById(userId, result.rows[0].id);
    return habit!;
  },

  async update(userId: string, id: string, input: UpdateHabitInput): Promise<Habit | null> {
    ...
    await pool.query(
      `UPDATE habits SET ${updates.join(', ')} WHERE id = $${idParam} AND user_id = $${userIdParam}`,
      values
    );
  ...
  async toggleComplete(userId: string, id: string): Promise<Habit | null> {
    const existing = await this.findById(userId, id);
    if (!existing) return null;
    ...
    if (existingCompletion.rows.length > 0) {
      ...
      await pool.query(
        `DELETE FROM habit_completions WHERE habit_id = $1 AND completed_date = $2 AND user_id = $3`,
        [id, today, userId]
      );
    } else {
      // Insert new completion
      await pool.query(
        `INSERT INTO habit_completions (user_id, habit_id, completed_date) VALUES ($1, $2, $3)`,
        [userId, id, today]
      );
    }

    // Recalculate streak
    const streak = await this.calculateStreak(userId, id);

    // Update best_streak if current streak exceeds it
    await pool.query(
      `UPDATE habits SET streak = $1, best_streak = GREATEST(best_streak, $1) WHERE id = $2 AND user_id = $3`,
      [streak, id, userId]
    );
  ...
  async getStats(userId: string): Promise<{
    ...
    const habitsForTodayResult = await pool.query<{ id: string }>(
      `SELECT id FROM habits WHERE $1 = ANY(target_days) AND user_id = $2`,
      [dayOfWeek, userId]
    );
    ...
    if (habitIdsForToday.length > 0) {
      const completedResult = await pool.query<{ count: string }>(
        `SELECT COUNT(*) as count FROM habit_completions
         WHERE habit_id = ANY($1) AND completed_date = $2 AND user_id = $3`,
        [habitIdsForToday, todayStr, userId]
      );
    ...
    const streakResult = await pool.query<{ max_streak: string; max_best_streak: string }>(
      `SELECT COALESCE(MAX(streak), 0) as max_streak, COALESCE(MAX(best_streak), 0) as max_best_streak FROM habits WHERE user_id = $1`,
      [userId]
    );
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const expectedResult = await pool.query<{ count: string }>(
        `SELECT COUNT(*) as count FROM habits WHERE $1 = ANY(target_days) AND user_id = $2`,
        [checkDayOfWeek, userId]
      );
      ...
      const actualResult = await pool.query<{ count: string }>(
        `SELECT COUNT(*) as count FROM habit_completions WHERE completed_date = $1 AND user_id = $2`,
        [checkDateStr, userId]
      );
