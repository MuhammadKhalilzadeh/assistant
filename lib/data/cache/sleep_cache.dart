...
class SleepCache {
  ...
  /// Get cached sleep records if they exist, are not expired, and match the date
  Future<List<SleepRecordModel>?> getCachedRecords({DateTime? date}) async {
    ...
    try {
      return recordsJson
          .map((e) => SleepRecordModel.fromJson(Map<String, dynamic>.from(e as Map)))
          .toList();
    } catch (_) {
  ...
  /// Cache sleep records with current timestamp
  Future<void> cacheRecords(List<SleepRecordModel> records, {DateTime? date}) async {
    final box = _recordsBox ?? await Hive.openBox(_recordsBoxName);
