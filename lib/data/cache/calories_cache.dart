...
class CaloriesCache {
  ...
  /// Get cached calorie entries if they exist, are not expired, and match the date
  Future<List<CalorieEntryModel>?> getCachedEntries({DateTime? date}) async {
    ...
    try {
      return entriesJson
          .map((e) => CalorieEntryModel.fromJson(Map<String, dynamic>.from(e as Map)))
          .toList();
    } catch (_) {
  ...
  /// Cache calorie entries with current timestamp
  Future<void> cacheEntries(List<CalorieEntryModel> entries, {DateTime? date}) async {
    final box = _entriesBox ?? await Hive.openBox(_entriesBoxName);
