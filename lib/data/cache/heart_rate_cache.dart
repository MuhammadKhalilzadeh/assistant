...
class HeartRateCache {
  ...
  /// Get cached heart rate records if they exist, are not expired, and match the date
  Future<List<HeartRateRecordModel>?> getCachedRecords({DateTime? date}) async {
    ...
    try {
      return recordsJson
          .map((e) => HeartRateRecordModel.fromJson(Map<String, dynamic>.from(e as Map)))
          .toList();
    } catch (_) {
  ...
  /// Cache heart rate records with current timestamp
  Future<void> cacheRecords(List<HeartRateRecordModel> records, {DateTime? date}) async {
    final box = _recordsBox ?? await Hive.openBox(_recordsBoxName);
