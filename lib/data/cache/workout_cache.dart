...
class WorkoutCache {
  ...
  /// Get cached workout sessions if they exist, are not expired, and match the date
  Future<List<WorkoutSessionModel>?> getCachedSessions({DateTime? date}) async {
    ...
    try {
      return sessionsJson
          .map((e) => WorkoutSessionModel.fromJson(Map<String, dynamic>.from(e as Map)))
          .toList();
    } catch (_) {
  ...
  /// Cache workout sessions with current timestamp
  Future<void> cacheSessions(List<WorkoutSessionModel> sessions, {DateTime? date}) async {
    final box = _sessionsBox ?? await Hive.openBox(_sessionsBoxName);
